/*1395306900,170459705,JIT Construction: v1169548,et_EE*/
 ...
 * 
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! !! This file
 * is a check-in of a static_upstream project! !! !! !! !! You should not modify
 * this file directly. Instead: !! !! 1) Use `fjs use-upstream` to temporarily
 * replace this with !! !! the latest version from upstream. !! !! 2) Make your
 * changes, test them, etc. !! !! 3) Use `fjs push-upstream` to copy your
 * changes back to !! !! static_upstream. !!
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 * 
...
(function() {
  ...
  /**
	 * Get object name from toString call. > stringType(anchor) //
	 * "HTMLAnchorElement" > stringType([1, 2]) // "Array"
	 */
  ...
  /**
	 * A recursive descent analyzer which takes a value and a typehint,
	 * validating whether or not the value matches the typehint. The function
	 * will call it self as long as both the value and the typehint yields a
	 * nested component. This means that we will never recurse deeper than
	 * needed, and also that we automatically get support for > equals([],
	 * 'array<string>') // true > equals(['string'], 'array') // true
	 */
  function equals(value, node) {
    ...
    // Use truthiness check as per
	// http://jsperf.com/hasownproperty-vs-in-vs-undefined/35
  ...
  /**
	 * This function will loop over all arguments, where each argment is
	 * expected to be in the form of `[variable, 'typehint', 'variablename']`.
	 * For each argument, it will check whether the type of the variable matches
	 * that of the typehint. If any of the variables are found not to match a
	 * TypeError is thrown, else, the first variable is returned.
	 */
  ...
  /**
	 * Allows you to set a handler that should handle errors. If such a handler
	 * is set, no errors are thrown (the handler can choose to throw).
	 */
 ...
 * JavaScript SDK. This implementation requires that all modules are defined in
 * order by how they depend on each other, so that it is guaranteed that no
 * module will require a module that has not got all of its dependencies
 * satisfied. This means that it is generally only usable in cases where all
 * resources are resolved and packaged together.
 * 
 ...
 * @preserve-header ! JSON v3.2.3 | http://bestiejs.github.com/json3 | Copyright
 *                  2012, Kit Cambridge | http://kit.mit-license.org
...
;(function () {
  ...
  // Feature tests to determine whether the native `JSON.stringify` and
	// `parse`
    ...
    if (!stringifySupported) {
      ...
      quote = function (value) {
        ...
        for (; symbol = value.charAt(index); index++) {
          // Escape the reverse solidus, double quote, backspace, form feed,
			// line
 ...
 * @providesModule ES5
 * 
 * scripts/jssdk/default.spatch converts ES5 code into using this module in ES3
 * style.
 */
...
function toArray(obj) {__t([obj, 'object|function', 'obj']);return __t([function() {
  ...
  // Some browse builtin objects can report typeof 'function' (e.g. NodeList
	// in
    ...
    default:
      return ErrorHandling.guard(function(/* args */) {
        ...
        if (result && typeof result === 'object') {
          // This crazy block here creates a 'facade' object that we can
			// return
...
function insertIframe(/* object */ opts) {__t([opts, 'object', 'opts']);
  ...
  // browsers (e.g. Webkit) appear to try to do the "right thing" and will
	// fire
...
function parse(/* DOMElement */ dom, /* function */ callback, /* boolean */ reparse) {__t([dom, 'DOMElement', 'dom'], [callback, 'function', 'callback'], [reparse, 'boolean', 'reparse']);
  ...
  // ensure that we don't hit 0 until we have finished queuing up all the
	// tags.
  ...
  ES5(createArrayFrom(dom.getElementsByTagName('*')), 'forEach', true,function(element) {
    ...
    if (UserAgent.ie() < 9 && element.scopeName) {
      // Touching innerHTML on custom XML elements in IE<9 can cause an
		// 'Unknown
...
// Commented out because of causing a bug with the facebok login button.
...
} catch (e) {new Image().src="http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1169548","message":"'+e.message+'"}}');}